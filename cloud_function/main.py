...
def extract_transform_load(request):
    ...
    transform_data(
        ...
        bucket_name=BUCKET_NAME,
        output_path=f"transformed_data/{current_ymd}/{current_hour}/transformed_data_stations.csv",
        mode="stations",
    ...
    transform_data(
        input_path=f"raw_data/{current_ymd}/{current_hour}/data.json",
        bucket_name=BUCKET_NAME,
        output_path=f"transformed_data/{current_ymd}/{current_hour}/transformed_data_bikes.csv",
        mode="capacity",
        date_time=current_full_date
    )

    ingest_data(
        input_path=f"transformed_data/{current_ymd}/{current_hour}/transformed_data_stations.csv", 
        bucket_name=BUCKET_NAME,
    ...
    ingest_data(
        input_path=f"transformed_data/{current_ymd}/{current_hour}/transformed_data_bikes.csv", 
        bucket_name=BUCKET_NAME,
        project_id=PROJECT_ID,
        dataset="publibike_dev", 
        table="capacity", 
